...
export default async function TopRatedProducts() {
    const productsCollection = dbConnect("products") 
        const topProducts = await productsCollection.find({}).sort({ ratings: -1 })
    .limit(8).toArray();
  return (
    ...
    {topProducts.map((product) => (
      ...
      >
        ...
        <div className="p-4 flex flex-col items-center text-center">
          ...
          <button className="mt-4 w-full py-2 bg-blue-600 text-white font-medium rounded-md hover:bg-blue-700 transition-colors duration-200">
            View Details
          </button>
        </div>
      </div>
